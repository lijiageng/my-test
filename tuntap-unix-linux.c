...
tuntap_sys_start(struct device *dev, int mode, int tun) {
	...
	if (persist == 1) {
		...
		tuntap_log(TUNTAP_LOG_ERR, "set persistent");
	}	
	...
	/*
	unsigned int flag_offload = TUN_F_CSUM | TUN_F_TSO4 | TUN_F_TSO6 | TUN_F_TSO_ECN | TUN_F_UFO;
	if (ioctl(fd, TUNSETOFFLOAD, flag_offload) == -1) {
		tuntap_log(TUNTAP_LOG_ERR, "Can't set offload");
		return -1;
	}
	*/
